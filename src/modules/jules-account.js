// ===== Jules Account/Profile UI Module =====
// Used by the /profile and /jules pages to load account info.
...
async function loadAndDisplayJulesProfile(uid) {
  ...
  try {
    ...
    if (!profileData) {
      if (sourcesListDiv) sourcesListDiv.innerHTML = '<div style="color:var(--muted); font-size:13px;">Loading sources...</div>';
      if (sessionsListDiv) sessionsListDiv.innerHTML = '<div style="color:var(--muted); font-size:13px;">Loading sessions...</div>';
    ...
    if (sourcesListDiv) {
      if (profileData.sources && profileData.sources.length > 0) {
        const sourcesHtml = profileData.sources
          .map((source, index) => {
            ...
            const branchesHtml =
              branches.length > 0
               ...
               ${branches
                 .map(
                   (b) => `<div style="padding:3px 0 3px 8px; cursor:pointer;"
                  onclick="window.open('https://github.com/${githubPath}/tree/${encodeURIComponent(
        ...
        sourcesListDiv.innerHTML =
          '<div style="color:var(--muted); font-size:13px; text-align:center; padding:16px;">No connected repositories found.<br><small>Connect repos in the Jules UI.</small></div>';
      }
    }

    if (sessionsListDiv) {
      if (profileData.sessions && profileData.sessions.length > 0) {
        const sessionsHtml = profileData.sessions
          .map((session) => {
            ...
            const displayPrompt =
              promptPreview.length < (session.prompt || '').length ? promptPreview + '...' : promptPreview;
        ...
        sessionsListDiv.innerHTML =
          '<div style="color:var(--muted); font-size:13px; text-align:center; padding:16px;">No recent sessions found.</div>';
      }
    }
...
export function showJulesSessionsHistoryModal() {
  ...
  modal.setAttribute(
    'style',
    'display: flex !important; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.7); z-index:1002; flex-direction:column; align-items:center; justify-content:center; overflow-y:auto; padding:20px;'
  );
...
export function hideJulesSessionsHistoryModal() {
  ...
  modal.setAttribute(
    'style',
    'display: none !important; position:fixed; top:0; left:0; right:0; bottom:0; background:rgba(0,0,0,0.7); z-index:1002; flex-direction:column; align-items:center; justify-content:center; overflow-y:auto; padding:20px;'
  );
}
...
async function loadSessionsPage() {
  ...
  try {
    ...
    if (allSessionsCache.length === 0 && allSessionsList) {
...
function renderAllSessions(sessions) {
  const allSessionsList = document.getElementById('allSessionsList');
  const searchInput = document.getElementById('sessionSearchInput');
  if (!allSessionsList || !searchInput) return;
  ...
  const filteredSessions = searchTerm
    ? sessions.filter((s) => {
        ...
        return (
          promptText.toLowerCase().includes(searchTerm) || sessionId.toLowerCase().includes(searchTerm)
        );
      })
    : sessions;
  ...
  if (filteredSessions.length === 0) {
    allSessionsList.innerHTML =
      '<div style="color:var(--muted); text-align:center; padding:24px;">No sessions match your search</div>';
    return;
  }
  ...
  allSessionsList.innerHTML = filteredSessions
    .map((session) => {
      ...
      const subtaskInfo = subtaskCount > 0
        ? `<div style="font-size:11px; color:var(--muted); margin-top:4px;">📋 ${subtaskCount} subtask${
            subtaskCount > 1 ? 's' : ''
          }</div>`
        : '';
      ...
      <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:6px;">
        ...
        <div style="font-size:11px; color:var(--muted);">${emoji} ${label}</div>
      </div>
      <div style="font-size:11px; color:var(--muted);">Created: ${createTime}</div>
