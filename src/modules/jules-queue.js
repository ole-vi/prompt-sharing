...
export async function addToJulesQueue(uid, queueItem) {
  ...
  try {
    const collectionRef = getDb().collection('julesQueues').doc(uid).collection('items');
...
async function unscheduleSelectedQueueItems() {
  ...
  try {
    ...
    for (const docId of queueSelections) {
      const docRef = getDb().collection('julesQueues').doc(user.uid).collection('items').doc(docId);
      batch.update(docRef, {
        status: 'pending',
