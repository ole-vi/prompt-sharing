...
export async function callRunJulesFunction(promptText, sourceId, branch = 'master', title = '') {
  ...
  if (!user) {
    handleError(JULES_MESSAGES.NOT_LOGGED_IN, 'RunJules', { category: ErrorCategory.AUTH });
    return null;
  }
  ...
  try {
    ...
    if (!user) {
      try {
        ...
        handleError(JULES_MESSAGES.LOGIN_REQUIRED, 'TryInJulesLogin', { category: ErrorCategory.AUTH });
      }
      return;
    }
    await handleTryInJulesAfterAuth(promptText);
  } catch (error) {
    handleError(error, 'TryInJules');
  }
}
...
export async function handleTryInJulesAfterAuth(promptText) {
  const user = window.auth ? window.auth.currentUser : null;
  if (!user) {
    handleError(JULES_MESSAGES.NOT_LOGGED_IN, 'TryInJulesAfterAuth', { category: ErrorCategory.AUTH });
    return;
  }
  ...
  try {
    ...
    handleError(error, 'TryInJulesAfterAuth', { category: ErrorCategory.UNEXPECTED });
  }
}
