...
import { JULES_MESSAGES, JULES_UI_TEXT } from '../utils/constants.js';
import { showPromptViewer } from './prompt-viewer.js';
import { getFieldDelete, getServerTimestamp } from '../utils/firestore-helpers.js';
...
import {
  runSelectedQueueItems,
  deleteSelectedQueueItems,
  unscheduleSelectedQueueItems
} from './jules-queue-actions.js';
...
export async function openEditQueueModal(docId) {
...
export async function saveQueueItemEdit(docId, closeModalCallback) {
  const queueCache = getQueueCache();
  const item = queueCache.find(i => i.id === docId);
  if (!item) {
...
export async function loadScheduleModal() {
